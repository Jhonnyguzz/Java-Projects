...
public class ABT<T> implements BinaryTree{
	...
	public boolean isEmpty() {		
		return (arbol[1]==null)?true:false;
	}
	@Override
	public T root() {		
		return arbol[1];
	...
	public void makeTree(Object root, BinaryTree left, BinaryTree right) {
		 ...
		 if(rig!=null)this.arbol[3]=rig[1];
		 ...
		 for(int i=2;i<rig.length;i++){
			 if(rig[i]!=null){
				 ...
				 if(a>-1)
				 if(i%2==0){
				    this.arbol[a*2]=rig[i];				 
				    this.arbol[a*2+1]=rig[i+1];
				    i++;
				 }else{
					this.arbol[a*2+1]=rig[i]; 
				 }   
			 }
		 }	 
	}
